...
module.exports.userCollection = async (req,res) => {
    ...
    // books of each users
    const mybooks = await Book.find({owner : req.params._id})
    // console.log(mybooks)
    res.send(mybooks)
}
...
module.exports.addToWishList = async (req,res) => {
  ...
  const result = await Book.findOneAndUpdate({_id : req.body.book},{wishListedBy : updatedWishList})
  // console.log(result)
  res.send(result)
}
...
module.exports.getWishList = async (req,res) => {
  ...
  const wishBooks = books.filter((book) => book.wishListedBy?book.wishListedBy.includes(req.user._id) : false)
